...
async function loadSections() {
  ...
  // ── ALL SLIDERS INIT HERE — after sections are in the DOM ──
  initFeatureSlider();
...
// ── FEATURE SLIDER (Platform Features section) ──
function initFeatureSlider() {
  ...
  function goTo(index) {
    ...
    track.style.transform = `translateX(-${current * (slides[0].offsetWidth + 24)}px)`;
    updateDots();
  }

  function slide(dir) {
    let next = current + dir;
    if (next >= totalSteps()) next = 0;
...
// ── GENERIC RESPONSIVE SLIDER (How It Works, Core Values) ──
function initResponsiveSlider(trackId, prevId, nextId, dotsId) {
  ...
  prevBtn.addEventListener("click", () => {
    goTo(current - 1 < 0 ? totalSteps() - 1 : current - 1);
  });
  nextBtn.addEventListener("click", () => {
    goTo(current + 1 >= totalSteps() ? 0 : current + 1);
  });
  ...
  track.addEventListener("touchend", (e) => {
    ...
    if (Math.abs(diff) > 40) {
      goTo(diff > 0
        ? current + 1 >= totalSteps() ? 0 : current + 1
        : current - 1 < 0 ? totalSteps() - 1 : current - 1
      );
    }
  }, { passive: true });
...
function initCoursesSlider(trackId) {
  ["coursesTrack-all", "coursesTrack-bachelor", "coursesTrack-masters"].forEach((id) => {
