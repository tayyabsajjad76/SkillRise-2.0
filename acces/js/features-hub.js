...
async function generateRoadmap() {
  ...
  const field = (fieldEl?.value.trim()) || window._userProfile?.field || "Full Stack Web Development";
  if (!field) { alert("Please enter a field first!"); return; }

  btn.textContent = "⏳ Generating..."; btn.disabled = true;

  const existingSkills = window._userProfile?.existingSkills || "";
  const level = window._userProfile?.level || "Beginner";

  try {
    const res = await fetch(`${API}/api/chat`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ message: `Generate a learning roadmap for "${field}". Student level: ${level}. Already knows: ${existingSkills || "nothing"}.
Return ONLY a JSON array, no markdown, no backticks:
[{"title":"Step Title","desc":"Short description","status":"done|active|locked","step":1}]
Generate 6-8 steps. First steps should be "done" if student already knows those skills, one step "active", rest "locked".` })
    });
    const data = await res.json();
    let steps;
    try {
    ...
    if (steps && Array.isArray(steps)) {
      renderRoadmapSteps(steps, field);
    }
  } catch(err) {
    const stepsEl = document.getElementById("roadmapSteps");
    if (stepsEl) stepsEl.innerHTML = `<p style="color:#ff4d6d;padding:20px;">❌ Could not generate roadmap. Try again.</p>`;
  }
  btn.textContent = "🤖 Generate AI Roadmap"; btn.disabled = false;
}

function renderRoadmapSteps(steps, field) {
  const stepsEl = document.getElementById("roadmapSteps");
  ...
  if (descEl) descEl.textContent = `AI-built roadmap for ${field}. ${done}/${total} milestones complete.`;
  if (badgeEl) badgeEl.textContent = `${done} / ${total} Done`;

  stepsEl.innerHTML = steps.map((s, i) => {
    const isLast = i === steps.length - 1;
    let dotHTML, badgeHTML;

    if (s.status === "done") {
      dotHTML = `<div class="rm-dot done"><i class="fa-solid fa-check"></i></div>`;
      badgeHTML = `<span class="badge badge-green">Completed</span>`;
    } else if (s.status === "active") {
      dotHTML = `<div class="rm-dot active">${s.step}</div>`;
      badgeHTML = `<span class="badge badge-blue">In Progress</span>`;
    } else {
      dotHTML = `<div class="rm-dot locked"><i class="fa-solid fa-lock"></i></div>`;
      badgeHTML = `<span class="badge badge-orange">Locked</span>`;
    }

    return `<div class="rm-step">
      <div class="rm-line">
        ${dotHTML}
        ${!isLast ? `<div class="rm-connector ${s.status === "done" ? "done" : ""}"></div>` : ""}
      </div>
      <div class="rm-body">
        <h4>${s.title}</h4>
        <p>${s.desc}</p>
        <div class="rm-meta">${badgeHTML}</div>
      </div>
    </div>`;
  }).join("");
}
function autoLoadRoadmap(profile) {
  if (!profile || !profile.field) return;
  const fieldEl = document.getElementById("roadmapField");
  if (fieldEl) fieldEl.value = profile.field;
  generateRoadmap();
}
