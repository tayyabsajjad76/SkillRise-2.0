...
async function submitInterviewAnswer() {
  ...
  try {
    ...
    if (comm !== null || tech !== null) {
      const topicEl = document.getElementById("interviewTopic");
      saveInterviewSession({ comm, tech, conf, topic: topicEl?.value.trim() || "General", date: new Date().toISOString() });
    }
...
function renderCurrentProject(proj) {
  ...
  // Update home project card
  const homeProjectText = document.getElementById("homeProjectText");
  const homeProjectBar  = document.getElementById("homeProjectBar");
  if (homeProjectText) homeProjectText.textContent = proj.title + " — In Progress";
  if (homeProjectBar)  homeProjectBar.style.width  = "15%";
}
...
function renderInterviewStats(history) {
  ...
  if (techEl && validTech.length > 0) techEl.textContent = (validTech.reduce((a,b)=>a+b,0)/validTech.length).toFixed(1) + "/10";
  if (commEl && validComm.length > 0) commEl.textContent = (validComm.reduce((a,b)=>a+b,0)/validComm.length).toFixed(1) + "/10";
  ...
  if (trendEl && validConf.length >= 2) {
    ...
    const diff  = (last.reduce((a,b)=>a+b,0)/last.length) - (first.reduce((a,b)=>a+b,0)/first.length);
    ...
    trendEl.textContent = "→ 1 session"; trendEl.className = "val-blue";
  }
}
// ── COURSE SUGGESTIONS ──
...
function applyProfileToUI(profile, stats) {
  ...
  const roadmapPct= completedSkills.length > 0 ? Math.min(Math.round(completedSkills.length * 14), 100) : 0;
  const cDone     = Object.values(courseProgress).filter(v=>v>=100).length;
  ...
  // ── HERO ──
  const heroP = document.getElementById("heroSubtext");
  ...
  if (!homeReadiness && statVals[0]) statVals[0].textContent = readiness + "%";
  if (!homeStreak    && statVals[1]) statVals[1].textContent = streak + " 🔥";
  if (!homeQuizAvg   && statVals[2]) statVals[2].textContent = quizAvg + "%";
  if (!homeProjects  && statVals[3]) statVals[3].textContent = completedProjects.length;
