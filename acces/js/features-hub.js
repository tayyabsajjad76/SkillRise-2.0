...
const pageInfo = {
  ...
  interview: {
    t: "AI Mock Interview",
    s: "Practice interview sessions — Session 6/8",
  },
...
function goto(id) {
  ...
  if (
    id === "analytics" &&
    !document.getElementById("chartBars").children.length
  )
    buildChart();
  if (id === "quiz") initQuiz();
}
...
const questions = [
  {
    ...
    ans: 1,
    exp: "The `async` keyword wraps the function's return value in a Promise automatically.",
  },
  {
    q: "What does `await` do inside an async function?",
    opts: [
      "Delays code by 1 second",
      "Catches errors",
      "Pauses until the Promise resolves",
      "Runs code in background",
    ],
    ans: 2,
    exp: "`await` pauses the async function until the awaited Promise resolves or rejects.",
  },
  {
    q: "Which correctly handles a Promise error?",
    opts: [
      ".error()",
      "try/catch or .catch()",
      "Promise.reject()",
    ...
    ans: 1,
    exp: "Use .catch() chained to a Promise, or try/catch inside an async function.",
  },
  {
    ...
    ans: 2,
    exp: "Promise.all() resolves when ALL promises in the array resolve.",
  },
  {
    ...
    ans: 1,
    exp: "Awaiting a non-Promise simply returns the value — treated as already resolved.",
  },
];
...
function pick(i, el) {
  ...
  fb.style.cssText = `
    ...
    font-size:.84rem;
    line-height:1.6;
    background:${ok ? "rgba(0,229,160,.08)" : "rgba(255,77,109,.08)"};
    border:1px solid ${ok ? "rgba(0,229,160,.2)" : "rgba(255,77,109,.2)"};
...
function updateResume() {
  ...
  document.getElementById("p-skills").textContent =
    document.getElementById("rv-skills").value;
  document.getElementById("p-edu").textContent =
    document.getElementById("rv-edu").value;
  const projs = document
    .getElementById("rv-projects")
    .value.split("\n")
    .filter((l) => l.trim());
  document.getElementById("p-projects").innerHTML = projs
    .map((p) => {
      const pts = p.split("—");
...
document
  .getElementById("resumePreviewBtn")
  .addEventListener("click", updateResume);
