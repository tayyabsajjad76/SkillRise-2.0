...
async function submitOnboarding() {
  ...
  // Save with retry (Render cold start can be slow)
  const doSave = async () => {
    const email = getCurrentUserEmail();
    const res = await fetch(`${API}/api/saveProfile`, {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email, profile })
    });
  ...
  try {
    ...
    try {
      ...
      alert("❌ Could not save profile. Check connection and try again.");
      btn.disabled = false; btn.textContent = "🚀 Generate My Learning Plan";
      if (loading) loading.style.display = "none";
      return;
    }
  }
 
  // Verify MongoDB actually saved it
  const verify = await fetchUserProfile();
  if (!verify?.profile?.field) {
...
Generate a motivating 2-sentence welcome message and their top 3 immediate action steps. Keep it short and encouraging.` })
  ...
  // Close modal — profile confirmed saved
...
// // ── PROJECT FUNCTIONS ──
// // ── PROJECTS ──
// let _currentProject = null; // cache in memory
...
//   // Load or generate current project
//   if (profile.currentProject) {
//     _currentProject = profile.currentProject;
//     renderCurrentProject(profile.currentProject);
//   } else {
//     await generateNewProject();
//   }
// }
...
//   // Build requirements text from array or string
//   if (reqText && proj.requirements) {
//     if (Array.isArray(proj.requirements)) {
//       reqText.innerHTML = proj.requirements.map((r, i) => `${i+1}. ${r}`).join("<br>");
//     } else {
//       reqText.innerHTML = proj.requirements;
//     }
//   }
//   if (reqBox) reqBox.style.display = "none";
// }
...
//     const res = await fetch(`${API}/api/chat`, {
//       method: "POST", headers: { "Content-Type": "application/json" },
//       body: JSON.stringify({ message: `Generate a practical coding project for a student with these details:
// - Field: ${profile.field || "Software Development"}
// - Level: ${profile.level || "Beginner"}
// - Career Goal: ${profile.goal || "Get a job"}
// - Existing Skills: ${profile.existingSkills || "Basic programming"}
// - Already completed projects: ${completed}

// Return ONLY valid JSON, no markdown, no backticks:
// {"title":"project title","description":"2 sentence project description","requirements":["requirement 1","requirement 2","requirement 3","requirement 4","requirement 5"]}

// Make it different from already completed projects. Make it realistic and achievable in 1-2 weeks.` }),
//     });
//     const resp = await res.json();
//     let parsed;
//     try {
...
//       renderCurrentProject(parsed);
//     } else {
//       if (titleEl) titleEl.textContent = "❌ Could not generate. Try again.";
//     }
//   } catch (err) {
//     if (titleEl) titleEl.textContent = "❌ Error generating project. Try again.";
//   }
